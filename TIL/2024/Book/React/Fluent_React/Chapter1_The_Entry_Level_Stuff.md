...
- React の価値提案はコンポーネントベースのアーキテクチャ、宣言的プトグラミングモデル、仮想 DOM、JSX、広範なエコシステム、プラットフォームにとらわれない性質、Meta によるバックアップが中心となっている

  - React は高速でスケーラブル、保守性の高い Web アプリケーションを構築する必要がある開発者にとってとても魅力的な選択肢となる
  - シンプルな Web サイトや複雑なエンタープライズ向けのアプリケーションを構築する場合でも、React は多くの他の技術よりもより効率的に目標に辿り着くことができるでしょう

- この章では、React の簡単な歴史、最初の価値提案、そして、安全でなく、予測不可能で、非効率な UI の更新をスケールで解決する方法説明した。コンポーネントモデルについて、なぜそれがウェブ上のインターフェースにとって革命的なものであるのかについても話してみました。
- この章を読み終えた後に React のルーツとその由来、そして主な強みと価値提案についてより深く理解できていることが理想です。

## レビュークエスチョン

1. React を作ろうと思った動機は?
2. React は MVS や MVVM のような prior パターンはどのように改善しているのか
3. Flux Architecture の特徴はなんですか
4. 宣言的プログラミング abstraction のメリットはなんですか
5. 効率的 UI 更新における仮想 DOM の役割はなんですか
