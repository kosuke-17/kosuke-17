...
- 同じ新機能の中でも重要な１つ機能
  - React16 で React.lazy によるコード分割のために Suspense は導入されている
  - React18 で導入された新機能によりデータフェッチまで拡張されます
- 読み込み状態を宣言的に定義することで、条件付きレンダリングロジックの必要性が減ります
- React Server Components に Suspense を組み合わせると、データベースやファイルシステムなどの別の API エンドポイントを必要とせずにサーバー側のデータソースに直接アクセスできます
...
- RSC と Suspense がシームレスに連携して、コンポーネントが読み込み中の間は読み込み状態を定義できます

- Suspense は React18 で導入された他の機能と複合的に使用することで正しい威力を発揮します

  - データを読み込んでいる間の待ち時間でコンポーネントが一時停止している場合、React はコンポーネントがデータを受け取るまでアイドル状態になるわけではない
  - 代わりに、一時停止されたコンポーネントのレンダリングを一時停止し、他のタスクにフォーカスします
  - この間、このコンポーネントがまだローディング中であることを示すフォールバック UI をレンダリングするように React に指示できます
  - 待機していたデータが利用可能になると、React は前にトランジションで発生したのと同じように、中断されたコンポーネントのレンダリングを中断可能な方法でシームレスに再開します
  - React はユーザー操作をもとにコンポーネントの優先順位を変更することができる
    - 現在レンダリングされていない一時停止されたコンポーネントをユーザーが操作する場合などで、React は進行中のレンダーを一時停止してユーザーが操作しているコンポーネントを優先します
    - 準備が完了すると、React は DOM をコミットして、前のレンダリングを再開します
    - これにより、ユーザー操作は優先されて、ユーザー入力に対して最新の状態が保たれます

- Suspence と React Server Components のストリーミング可能な形式を組み合わせることで、優先度の低いレンダリングタスクが完了するのを待たずに準備が整うとすぐに優先度の高い更新をクライアントに更新できます
- これにより、クライアントはデータの処理をより早く開始できるようになり、ノンブロッキングな方法でコンテンツが到着すると徐々に表示されるため、より流動的な UX が提供されます
- この中断可能なレンダリングメカニズムと Suspense の非同期操作を処理する機能を組み合わせることで、特にデータ取得の必要性が高い複雑なアプリケーションにおいて、よりスムーズでよりユーザー中心のエクスペリエンスが提供されます
